...
def solve(c: np.array, d: np.array, alpha: float, eps: float, x: np.array):
    ...
    print(f"\tВ качестве первых двух приближений рассмотрим\n"
          f"\t\tx0 = {x0}\n"
          f"\t\tx1 = {x1}\n"
          f"\t(dist(x0, x1) = {np.linalg.norm(x0 - x1)} -- расстояние между x0 и x1)")
...
def main():
    ...
    x = np.array([  # точное решение системы (получено методом Гаусса; можно сравнить с выводом Wolfram Alpha ^^^)
        10,
        6,
        20,
        35
